...
/*   Updated: 2024/11/28 13:44:59 by ipuig-pa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (line)
	{
		...
		while (column[x])
		{
			...
			if (ft_strchr(column[x], ','))
			{
				...
				env->coord[x + y].color = ft_atoi(*(ft_split(column[x], ',') + 1));
				...
				env->coord[x + y].z = ft_atoi(column[x]);
			env->coord[x + y].x = x;
			env->coord[x + y].y = y;
			//ft_printf("x: %i, y: %i, z: %i\n", env->coord[x + y].x, env->coord[x + y].y, env->coord[x + y].z);
			x++;
		}
		line = get_next_line(fd);
		y++;
...
void	find_final_coordinates(t_env *env)
{
	...
	{
		env->fcoord[i].x = 90 / 11 * env->coord[i].x * cos(M_PI / 6) - (90 / 11 * env->coord[i].y * sin(M_PI / 3));
		env->fcoord[i].y = 90 / 11 * env->coord[i].y * sin(M_PI / 6) + (90/11 * env->coord[i].x * sin(M_PI / 6)) - 90 / 11 * env->coord[i].z;
